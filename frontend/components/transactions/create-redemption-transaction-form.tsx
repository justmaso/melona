...
interface CreateRedemptionTransactionFormProps {
...
export function CreateRedemptionTransactionForm({
    register,
    errors,
    onSubmit,
    loading,
    ...
    success,
    countdown
}: CreateRedemptionTransactionFormProps) {
    const { user } = useAuth()
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-4">
            ...
            <input type="hidden" {...register("type")} value="redemption" />
            ...
            <Field>
                <Button type="submit" disabled={loading}>
                    {loading ? "Creating request..." : "Request redemption"}
                </Button>
            </Field>
        </form>
    )
