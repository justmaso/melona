...
export default function TransactionPage({ params }: { params: Promise<{ transactionId: string }> }) {
    ...
    useEffect(() => {
        async function loadTransaction() {
            try {
                if (!Permissions.viewTransaction(auth.user?.role as Role)) {
                    notFound()
                }

                const data = await TransactionService.get(Number(transactionId))
                setTransaction(data)
            } catch {
                notFound()
            } finally {
    ...
    const handleProcessRedemption = async () => {
        ...
        try {
            ...
            setTransaction((prev) => prev && { ...prev, processedBy: auth.user!.utorid })
        } finally {
            setProcessing(false)
        }
    }
