        ...
        <div className="w-full space-y-4">
            <div className="flex align-items justify-between gap-2">
                <Input
                    ...
                    value={(table.getColumn("utorid")?.getFilterValue() as string) ?? ""}
                    onChange={(event) =>
                        table.getColumn("utorid")?.setFilterValue(event.target.value)
                    }
                />
                <CreateTransactionDropdownMenu viewingFromMyTransactions={false} />
            </div>
                    ...
                    <TableBody>
                        {table.getRowModel().rows.length ? (
                            table.getRowModel().rows.map((row) => {
                                ...
                                const route = original.id ? `/dashboard/transactions/${original.id}` : ""
